 ...
 *
 * This is a modified version of al_DefaultShaders.cpp that uses
 * GLSL ES 3.0 syntax (#version 300 es) instead of desktop GLSL 3.30.
 */
...
lighting_shader_uniforms al_get_lighting_uniform_locations(al::ShaderProgram& s,
  ...
  for (int i = 0; i < al_max_num_lights(); i += 1) {
    ...
    auto light_i = "light"s + std::to_string(i);
...
std::string multilight_vert_body_perlight(int num_lights) {
  ...
  std::string s = "\n";
  for (int i = 0; i < num_lights; i += 1) {
    auto li = "light"s + std::to_string(i);
