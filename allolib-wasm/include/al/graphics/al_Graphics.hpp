/**
 * AlloLib Studio Online - Shadow Graphics Header
 *
 * This header shadows the original al_Graphics.hpp to add WebGPU backend support.
 * It adds draw() and clear() methods to the Graphics class that route through
 * the GraphicsBackend when WebGPU is active.
 *
 * CMake includes allolib-wasm/include BEFORE allolib/include, so this takes precedence.
 */
...
public:
  ...
  // ─── WebGPU-aware clear methods (defined in al_Graphics_Web.cpp) ───────────
  // These replace the original inline implementations to support backend routing
  ...
  // ─── WebGPU-aware draw methods (hide RenderManager::draw) ──────────────────
  // These add backend routing while keeping the original WebGL2 path
  ...
  // ─── WebGPU-aware viewport methods (hide RenderManager::viewport) ──────────
  // These update the viewport stack but skip glViewport() in WebGPU mode

  void viewport(int left, int bottom, int width, int height);
  void viewport(const Viewport &v) { viewport(v.l, v.b, v.w, v.h); }
  void popViewport();

  // Bring back base class pushViewport() since our overloads would hide it
  using RenderManager::pushViewport;

  // Override pushViewport variants to use our WebGPU-safe viewport()
  void pushViewport(int l, int b, int w, int h) {
    RenderManager::pushViewport();
    viewport(l, b, w, h);
  }
  ...
  // use user made non-default shader
  ...
  // ─── Accessors for WebGPU backend integration ──────────────────────────────
  Color const& currentColor() const { return mColor; }
  Color const& currentTint() const { return mTint; }
...
// ─── Backend Integration Functions (defined in al_Graphics_Web.cpp) ─────────

class GraphicsBackend;

/// Set the graphics backend for all Graphics instances
