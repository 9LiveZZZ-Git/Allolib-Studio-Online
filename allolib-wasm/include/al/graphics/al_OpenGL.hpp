/**
 * AlloLib Web - Patched OpenGL Header for WebGL2
 *
 * This extends the original al_OpenGL.hpp with WebGL2-specific functionality.
 * Main additions:
 * - getPointSize() - retrieves the point size stored when pointSize() is called
 *   (since glPointSize() doesn't exist in WebGL2)
 */
...
/// NOTE: In WebGL2, only GL_FILL is supported
...
/// NOTE: In WebGL2, this stores the value for shader uniforms
/// (glPointSize doesn't exist - use gl_PointSize in vertex shader)
void pointSize(float size);

/// Get current point size value
/// Use this to set the al_PointSize shader uniform in WebGL2
/// Returns the value last set by pointSize(), default 1.0
float getPointSize();
